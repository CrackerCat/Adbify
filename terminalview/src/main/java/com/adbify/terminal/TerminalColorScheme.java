...
public final class TerminalColorScheme {
    ...
    /**
     * http://upload.wikimedia.org/wikipedia/en/1/15/Xterm_256color_chart.svg, but with blue color
     * brighter.
     */
    private static final int[] DEFAULT_COLORSCHEME = {
            ...
            // COLOR_INDEX_DEFAULT_FOREGROUND, COLOR_INDEX_DEFAULT_BACKGROUND and
            // COLOR_INDEX_DEFAULT_CURSOR:
            0xffffffff,
            0xff000000,
            0xffffffff
    };
        ...
        for (Map.Entry<Object, Object> entries : props.entrySet()) {
            ...
            if (colorValue == 0)
                throw new IllegalArgumentException(
                        "Property '" + key + "' has invalid color: '" + value + "'");
    ...
    /**
     * If the "cursor" color is not set by user, we need to decide on the appropriate color that
     * will be visible on the current terminal background. White will not be visible on light
     * backgrounds and black won't be visible on dark backgrounds. So we find the perceived
     * brightness of the background color and if its below the threshold (too dark), we use white
     * cursor and if its above (too bright), we use black cursor.
     */
    public void setCursorColorForBackground() {
        ...
        if (brightness > 0) {
            if (brightness < 130) mDefaultColors[TextStyle.COLOR_INDEX_CURSOR] = 0xffffffff;
            else mDefaultColors[TextStyle.COLOR_INDEX_CURSOR] = 0xff000000;
        }
    }
}
