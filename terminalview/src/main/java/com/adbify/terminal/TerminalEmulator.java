...
/**
 * Renders text into a screen. Contains all the terminal-specific knowledge and state. Emulates a
 ...
 *
 * <ul>
 *   <li>http://invisible-island.net/xterm/ctlseqs/ctlseqs.html
 *   <li>http://en.wikipedia.org/wiki/ANSI_escape_code
 *   <li>http://man.he.net/man4/console_codes
 *   <li>http://bazaar.launchpad.net/~leonerd/libvterm/trunk/view/head:/src/state.c
 *   <li>http://www.columbia.edu/~kermit/k95manual/iso2022.html
 *   <li>http://www.vt100.net/docs/vt510-rm/chapter4
 *   <li>http://en.wikipedia.org/wiki/ISO/IEC_2022 - for 7-bit and 8-bit GL GR explanation
 *   <li>http://bjh21.me.uk/all-escapes/all-escapes.txt - extensive!
...
public final class TerminalEmulator {
     ...
     * the upper-left corner of the screen, within the margins. The starting point for line numbers
     * depends on the current top margin setting. The cursor cannot move outside of the margins.
     * When DECOM is reset, the home cursor position is at the upper-left corner of the screen. The
     * starting point for line numbers is independent of the margins. The cursor can move outside of
     * the margins."
     */
    private static final int DECSET_BIT_ORIGIN_MODE = 1 << 2;
    /**
     * http://www.vt100.net/docs/vt510-rm/DECAWM: "If the DECAWM function is set, then graphic
     * characters received when the cursor is at the right border of the page appear at the
     * beginning of the next line. Any text on the page scrolls up if the cursor is at the end of
     * the scrolling region. If the DECAWM function is reset, then graphic characters received when
     * the cursor is at the right border of the page replace characters already on the page."
     */
     ...
     * lines (so that when the alternate screen buffer is active, you cannot scroll back to view
     * saved lines).
     *
    ...
    /**
     * Current foreground and background colors. Can either be a color index in [0,259] or a
     * truecolor (24-bit) value. For a 24-bit value the top byte (0xff000000) is set.
     *
     * @see TextStyle
     */
     ...
     * inserted, pushing existing text to the right. Characters moved past the right margin are
     * lost.
     */
     ...
     * standing there, a written character will be output in the last column, the cursor not moving
     * but this flag will be set. When outputting another character this will move to the next line.
     */
    private boolean mAboutToAutoWrap;
    /**
     * If the cursor blinking is enabled. It requires cursor itself to be enabled, which is
     * controlled byt whether {@link #DECSET_BIT_CURSOR_ENABLED} bit is set or not.
     */
    ...
    private int mEffect;
    /**
     * The number of scrolled lines since last calling {@link #clearScrollCounter()}. Used for
     * moving selection up along with the scrolling text.
     */
    ...
    public TerminalEmulator(
        ...
        mScreen =
                mMainBuffer =
                        new TerminalBuffer(
                                columns, getTerminalTranscriptRows(transcriptRows), rows);
    ...
    public void sendMouseEvent(int mouseButton, int column, int row, boolean pressed) {
        ...
        if (mouseButton == MOUSE_LEFT_BUTTON_MOVED
            ...
            mSession.write(
                    String.format(
                            "\033[<%d;%d;%d" + (pressed ? 'M' : 'm'), mouseButton, column, row));
        } else {
    ...
    private void resizeScreen() {
        ...
        mScreen.resize(
                mColumns, mRows, newTotalRows, cursor, getStyle(), isAlternateBufferActive());
        mCursorCol = cursor[0];
        mCursorRow = cursor[1];
    }
    ...
    public void setCursorStyle() {
        ...
        if (cursorStyle == null
                || !Arrays.asList(TERMINAL_CURSOR_STYLES_LIST).contains(cursorStyle))
    ...
    private void processByte(byte byteToProcess) {
        if (mUtf8ToFollow > 0) {
            ...
            if ((byteToProcess & 0b10000000)
                ...
                return;
            } else if ((byteToProcess & 0b11100000)
                    == 0b11000000) { // 110xxxxx, a two-byte sequence.
                mUtf8ToFollow = 1;
            } else if ((byteToProcess & 0b11110000)
                    == 0b11100000) { // 1110xxxx, a three-byte sequence.
                mUtf8ToFollow = 2;
            } else if ((byteToProcess & 0b11111000)
                    == 0b11110000) { // 11110xxx, a four-byte sequence.
                mUtf8ToFollow = 3;
            } else {
            ...
            default:
                ...
                switch (mEscapeState) {
                    ...
                    case ESC_CSI_DOLLAR:
                        ...
                        switch (b) {
                            case 'v': // ${CSI}${SRC_TOP}${SRC_LEFT}${SRC_BOTTOM}${SRC_RIGHT}${SRC_PAGE}${DST_TOP}${DST_LEFT}${DST_PAGE}$v"
                                ...
                                // "If Pbs is greater than Pts, or Pls is greater than Prs, the
                                // terminal ignores DECCRA.
                                // The coordinates of the rectangular area are affected by the
                                // setting of origin mode (DECOM).
                                // DECCRA is not affected by the page margins.
                                // The copied text takes on the line attributes of the destination
                                // area.
                                // If the value of Pt, Pl, Pb, or Pr exceeds the width or height of
                                // the active page, then the value
                                // is treated as the width or height of that page.
                                // If the destination area is partially off the page, then DECCRA
                                // clips the off-page data.
                                ...
                                int leftSource =
                                        Math.min(
                                                getArg(1, 1, true) - 1 + effectiveLeftMargin,
                                                mColumns);
                                // Inclusive, so do not subtract one:
                                int bottomSource =
                                        Math.min(
                                                Math.max(
                                                        getArg(2, mRows, true) + effectiveTopMargin,
                                                        topSource),
                                                mRows);
                                int rightSource =
                                        Math.min(
                                                Math.max(
                                                        getArg(3, mColumns, true)
                                                                + effectiveLeftMargin,
                                                        leftSource),
                                                mColumns);
                                ...
                                int destinationLeft =
                                        Math.min(
                                                getArg(6, 1, true) - 1 + effectiveLeftMargin,
                                                mColumns);
                                // int destinationPage = getArg(7, 1, true);
                                int heightToCopy =
                                        Math.min(mRows - destionationTop, bottomSource - topSource);
                                int widthToCopy =
                                        Math.min(
                                                mColumns - destinationLeft,
                            ...
                            case 'z': // ${CSI}$${TOP}${LEFT}${BOTTOM}${RIGHT}$z"
                                ...
                                // "Pch can be any value from 32 to 126 or from 160 to 255. If Pch
                                // is not in this range, then the
                                // terminal ignores the DECFRA command":
                                if ((fillChar >= 32 && fillChar <= 126)
                                        || (fillChar >= 160 && fillChar <= 255)) {
                                    // "If the value of Pt, Pl, Pb, or Pr exceeds the width or
                                    // height of the active page, the value
                                    // is treated as the width or height of that page."
                                    int top =
                                            Math.min(
                                    ...
                                    int bottom =
                                            Math.min(
                                                    getArg(argIndex++, mRows, true)
                                                            + effectiveTopMargin,
                                                    effectiveBottomMargin);
                                    int right =
                                            Math.min(
                                                    getArg(argIndex, mColumns, true)
                                                            + effectiveLeftMargin,
                                        ...
                                        for (int col = left - 1; col < right; col++)
                                            if (!selective
                                                ...
                                                mScreen.setChar(
                                                        ...
                                                        row,
                                                        fillChar,
                                                        keepVisualAttributes
                                                                ? mScreen.getStyleAt(row, col)
                                                                : style);
                                }
                                break;
                            ...
                            case 't': // "${CSI}${TOP}${LEFT}${BOTTOM}${RIGHT}${ATTRIBUTES}$t"
                                ...
                                boolean reverse = b == 't';
                                // FIXME: "coordinates of the rectangular area are affected by the
                                // setting of origin mode (DECOM)".
                                int top =
                                        Math.min(getArg(0, 1, true) - 1, effectiveBottomMargin)
                                                + effectiveTopMargin;
                                int left =
                                        Math.min(getArg(1, 1, true) - 1, effectiveRightMargin)
                                                + effectiveLeftMargin;
                                int bottom =
                                        Math.min(
                                ...
                                if (mArgIndex >= 4) {
                                    ...
                                    for (int i = 4; i <= mArgIndex; i++) {
                                        ...
                                        if (reverse && !setOrClear) {
                                            // Reverse attributes in rectangular area ignores
                                            // non-(1,4,5,7) bits.
                                        } else {
                    ...
                    case ESC_CSI_QUESTIONMARK_ARG_DOLLAR:
                        if (b == 'p') {
                            ...
                            if (mode == 47 || mode == 1047 || mode == 1049) {
                                ...
                                if (internalBit != -1) {
                                    value =
                                            isDecsetInternalBitSet(internalBit)
                                                    ? 1
                                                    : 2; // 1=set, 2=reset.
                                } else {
                                    Logger.logError(
                    ...
                    case ESC_CSI_ARGS_ASTERIX:
                        ...
                        if (b == 'x'
                                && (attributeChangeExtent >= 0 && attributeChangeExtent <= 2)) {
        ...
        switch (b) {
            case (byte) '\\':  // End of ESC \ string Terminator
                ...
                if (dcs.startsWith("$q")) {
                    ...
                    // Request Termcap/Terminfo String. The string following the "q" is a list
                    // of names encoded in
                    ...
                    // for terminfo colors), and TN for termcap name (or name for terminfo
                    // name).
                    // xterm responds with DCS 1 + r P t ST for valid requests, adding to P t an
                    // = , and the value of the
                    // corresponding string that xterm would send, or DCS 0 + r P t ST for
                    // invalid requests. The strings are
                    ...
                    // "<27> P 1 + r 6 b 7 5 = 1 B 5 B 4 1" where 0x1B 0x5B 0x41 = 27 91 65 =
                    // ESC [ A
                    // Xterm response in application cursor mode:
                    // "<27> P 1 + r 6 b 7 5 = 1 B 5 B 4 1" where 0x1B 0x4F 0x41 = 27 91 65 =
                    // ESC 0 A
                    ...
                    // See
                    // http://h30097.www3.hp.com/docs/base_doc/DOCUMENTATION/V40G_HTML/MAN/MAN4/0178____.HTM for what to
                    // respond, as well as
                    // http://www.freebsd.org/cgi/man.cgi?query=termcap&sektion=5#CAPABILITIES
                    // for
                        ...
                        if (part.length() % 2 == 0) {
                            ...
                            for (int i = 0; i < part.length(); i += 2) {
                                try {
                                    ...
                                    Logger.logStackTraceWithMessage(
                                            mClient,
                                            LOG_TAG,
                                            "Invalid device termcap/terminfo encoded name \""
                                                    + part
                                                    + "\"",
                                            e);
                                    continue;
                            ...
                            if (responseValue == null) {
                                switch (trans) {
                                    ...
                                    default:
                                        Logger.logWarn(
                                                mClient,
                                                LOG_TAG,
                                                "Unhandled termcap/terminfo name: '"
                                                        + trans
                                                        + "'");
                                }
                                ...
                                for (int j = 0; j < responseValue.length(); j++) {
                                    hexEncoded.append(
                                            String.format(
                                                    "%02X", (int) responseValue.charAt(j)));
                                }
                            ...
                            Logger.logError(
                                    mClient,
                                    LOG_TAG,
                                    "Invalid device termcap/terminfo name of odd length: "
                                            + part);
                        }
                    }
                } else {
                    if (LOG_ESCAPE_SEQUENCES)
                        Logger.logError(
                                mClient, LOG_TAG, "Unrecognized device control string: " + dcs);
                }
                finishSequence();
                break;
            default:
                ...
                for (int row = startRow; row < endRow; row++) {
                    for (int col = startCol; col < endCol; col++) {
                        if ((TextStyle.decodeEffect(mScreen.getStyleAt(row, col))
                                & TextStyle.CHARACTER_ATTRIBUTE_PROTECTED)
                                == 0) mScreen.setChar(col, row, fillChar, style);
                    }
                }
                break;
            case 'h':
    ...
    private void doCsiBiggerThan(int b) {
        switch (b) {
            case 'c': // "${CSI}>c" or "${CSI}>c". Secondary Device Attributes (DA2).
                // Originally this was used for the terminal to respond with "identification code,
                // firmware version level,
                // and hardware options" (http://vt100.net/docs/vt510-rm/DA2), with the first "41"
                // meaning the VT420
                // terminal type. This is not used anymore, but the second version level field has
                // been changed by xterm
                // to mean it's release number ("patch numbers" listed at
                // http://invisible-island.net/xterm/xterm.log.html),
                // and some applications use it as a feature check:
                // * tmux used to have a "xterm won't reach version 500 for a while so set that as
                // the upper limit" check,
            ...
            case 'm':
                ...
                // layouts (legacy and vt220). This is done to provide compatible keyboards for DEC
                // VT220 and related
    ...
    private void doEsc(int b) {
        switch (b) {
            ...
            case 'M': // "${ESC}M" - reverse index (RI).
                // http://www.vt100.net/docs/vt100-ug/chapter3.html: "Move the active position to
                // the same horizontal
                // position on the preceding line. If the active position is at the top margin, a
        ...
        switch (b) {
            ...
            case 'J': // "${CSI}${0,1,2,3}J" - Erase in Display (ED)
                ...
                switch (getArg0(0)) {
                    ...
                    case 2: // Erase all of the display - all lines are erased, changed to
                        // single-width, and the cursor does not
                        // move..
                        blockClear(0, 0, mColumns, mRows);
                        break;
            ...
            case 'T':
                if (mArgIndex == 0) {
                    ...
                    final int linesToScroll =
                            Math.min(linesBetweenTopAndBottomMargins, linesToScrollArg);
                    mScreen.blockCopy(
                            0,
                            mTopMargin,
                            mColumns,
            ...
            case 'X': // "${CSI}${N}X" - Erase ${N:=1} character(s) (ECH). FIXME: Clears character
                ...
                mScreen.blockSet(
                        mCursorCol,
                        mCursorRow,
                        Math.min(getArg0(1), mColumns - mCursorCol),
                        1,
                        ' ',
                        getStyle());
                break;
            ...
            case 'c': // Primary Device Attributes (http://www.vt100.net/docs/vt510-rm/DA1) if
                ...
                // is the first response parameter identifying the terminal service class, where we
                // send 64 for "vt420".
            ...
            case 'n': // Esc [ Pn n - ECMA-48 Status Report Commands
                ...
                switch (getArg0(0)) {
                    ...
                    case 6: // Cursor position report (CPR):
                        ...
                        mSession.write(
                                String.format(
                                        Locale.US, "\033[%d;%dR", mCursorRow + 1, mCursorCol + 1));
                        break;
                    default:
                        break;
                }
            ...
            case 't': // Window manipulation (from dtterm, as well as extensions)
                switch (getArg0(0)) {
                    case 11: // Report xterm window state. If the xterm window is open
                        // (non-iconified), it returns CSI 1 t .
            ...
            if (code == 0) { // reset
                ...
                if (LOG_ESCAPE_SEQUENCES)
                    Logger.logWarn(
                            mClient,
                            LOG_TAG,
                            String.format(Locale.ENGLISH, "SGR unknown code %d", code));
            }
        }
    }
    ...
    private void doOscSetTextParameters(String bellOrStringTerminator) {
        ...
        for (int mOSCArgTokenizerIndex = 0;
             mOSCArgTokenizerIndex < mOSCOrDeviceControlArgs.length();
             mOSCArgTokenizerIndex++) {
        ...
        switch (value) {
            ...
            case 2: // Change window title to T.
                ...
                break;
            case 4:
                // P s = 4 ; c ; spec → Change Color Number c to the color specified by spec. This
                // can be a name or RGB
                // specification as per XParseColor. Any number of c name pairs may be given. The
                // color numbers correspond
                ...
                // control sequence of the
                // same form which can be used to set the corresponding color. Because more than one
                // pair of color number
                // and specification can be given in one control sequence, xterm can make more than
                // one reply.
                int colorIndex = -1;
                ...
                for (int charIndex = 0; ; charIndex++) {
                    ...
                    if (endOfInput || textParameter.charAt(charIndex) == ';') {
                        try {
                            ...
                            if ("?".equals(colorSpec)) {
                                // Report current color in the same format xterm and gnome-terminal
                                // does.
                                ...
                                mSession.write(
                                        "\033]"
                                                + value
                                                + ";rgb:"
                                                + String.format(Locale.US, "%04x", r)
                                                + "/"
                                                + String.format(Locale.US, "%04x", g)
                                                + "/"
                                                + String.format(Locale.US, "%04x", b)
                            ...
                            if (endOfInput
                                    ...
                                    || ++charIndex >= textParameter.length()) break;
            ...
            case 104:
                ...
                // corresponding X
                // resource. Any number of c parameters may be given. These parameters correspond to
                // the ANSI colors 0-7,
                // their bright versions 8-15, and if supported, the remainder of the 88-color or
                // 256-color table. If no
                    ...
                    for (int charIndex = 0; ; charIndex++) {
                        ...
                        if (endOfInput || textParameter.charAt(charIndex) == ';') {
                            try {
                                int colorToReset =
                                        Integer.parseInt(
                                                textParameter.substring(lastIndex, charIndex));
    ...
    /**
     * NOTE: The parameters of this function respect the {@link #DECSET_BIT_ORIGIN_MODE}. Use {@link
     * #setCursorRowCol(int, int)} for absolute pos.
     */
    private void setCursorPosition(int x, int y) {
        ...
        int newCol =
                Math.max(
                        effectiveLeftMargin,
                        Math.min(effectiveLeftMargin + x, effectiveRightMargin - 1));
    ...
    private void scrollDownOneLine() {
        ...
        if (mLeftMargin != 0 || mRightMargin != mColumns) {
            // Horizontal margin: Do not put anything into scroll history, just non-margin part of
            // screen up.
            mScreen.blockCopy(
                    mLeftMargin,
                    mTopMargin + 1,
            ...
            mScreen.blockSet(
                    mLeftMargin, mBottomMargin - 1, mRightMargin - mLeftMargin, 1, ' ', mEffect);
        } else {
     ...
     *
     * <p>Parameter characters modify the action or interpretation of the sequence. You can use up
     * to 16 parameters per sequence. You must use the ; character to separate parameters. All
     * parameters are unsigned, positive decimal integers, with the most significant digit sent
     * first. Any parameter greater than 9999 (decimal) is set to 9999 (decimal). If you do not
     * specify a value, a 0 value is assumed. A 0 value or omitted parameter indicates a default
     * value for the sequence. For most sequences, the default value is 1.
     *
     * <p>https://vt100.net/docs/vt510-rm/chapter4.html#S4.3.3
     */
    private void parseArg(int inputByte) {
        ...
        if (mEscapeState == ESC_CSI) {
            if ((mIsCSIStart && inputByte == ';')
                    || // If sequence starts with a ; character, like \033[;m
                    (!mIsCSIStart
                            && mLastCSIArg != null
                            && mLastCSIArg == ';'
                            && inputByte
                            == ';')) { // If sequence contains sequential ; characters, like
                // \033[;;m
    ...
    private void emitCodePoint(int codePoint) {
        ...
        if (autoWrap) {
            if (cursorInLastColumn
                    && ((mAboutToAutoWrap && displayWidth == 1) || displayWidth == 2)) {
        ...
        int offsetDueToCombiningChar =
                ((displayWidth <= 0 && mCursorCol > 0 && !mAboutToAutoWrap) ? 1 : 0);
        ...
        // so was mCursorCol changed after the offsetDueToCombiningChar conditional by another
        // thread?
    ...
    /**
     * TODO: Better name, distinguished from {@link #setCursorPosition(int, int)} by not regarding
     * origin mode.
     */
    ...
    public void reset() {
        ...
        mSavedDecSetFlags =
                mSavedStateMain.mSavedDecFlags =
    ...
    static final class SavedScreenState {
        /**
         * Saved state of the cursor position, Used to implement the save/restore cursor position
         * escape sequences.
         */
