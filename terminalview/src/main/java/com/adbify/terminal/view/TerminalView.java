package com.adbify.terminal.view;
...
import com.adbify.terminal.KeyHandler;
import com.adbify.terminal.TerminalEmulator;
import com.adbify.terminal.TerminalSession;
import com.adbify.terminal.view.textselection.TextSelectionCursorController;
...
public final class TerminalView extends View {
    ...
    /**
     * The {@link KeyEvent} is generated from a virtual keyboard, like manually with the {@link
     * KeyEvent#KeyEvent(int, int)} constructor.
     */
    public static final int KEY_EVENT_SOURCE_VIRTUAL_KEYBOARD =
            KeyCharacterMap.VIRTUAL_KEYBOARD; // -1
    /**
     * The {@link KeyEvent} is generated from a non-physical device, like if 0 value is returned by
     * {@link KeyEvent#getDeviceId()}.
     */
    ...
    public TerminalView(Context context, AttributeSet attributes) { // NO_UCD (unused code)
        ...
        mGestureRecognizer =
                new GestureAndScaleRecognizer(
                        context,
                        new GestureAndScaleRecognizer.Listener() {
                            ...
                            public boolean onUp(MotionEvent event) {
                                ...
                                if (mEmulator != null
                                        ...
                                        && !isSelectingText()
                                        && !scrolledWithFinger) {
                                    // Quick event processing when mouse tracking is active - do not
                                    // wait for check of double tapping
                                    // for zooming.
                                    sendMouseEventCode(
                                            event, TerminalEmulator.MOUSE_LEFT_BUTTON, true);
                                    sendMouseEventCode(
                                            event, TerminalEmulator.MOUSE_LEFT_BUTTON, false);
                                    return true;
                                }
                            ...
                            public boolean onScroll(
                                    MotionEvent e, float distanceX, float distanceY) {
                                if (mEmulator == null) return true;
                                if (mEmulator.isMouseTrackingActive()
                                        && e.isFromSource(InputDevice.SOURCE_MOUSE)) {
                                    // If moving with mouse pointer while pressing button, report
                                    // that instead of scroll.
                                    // This means that we never report moving with button
                                    // press-events for touch input,
                                    // since we cannot just start sending these events without a
                                    // starting press event,
                                    // which we do not do for touch input, only mouse in
                                    // onTouchEvent().
                                    sendMouseEventCode(
                                            e, TerminalEmulator.MOUSE_LEFT_BUTTON_MOVED, true);
                                } else {
                                    scrolledWithFinger = true;
                                    distanceY += mScrollRemainder;
                                    int deltaRows = (int) (distanceY / mRenderer.mFontLineSpacing);
                                    mScrollRemainder =
                                            distanceY - deltaRows * mRenderer.mFontLineSpacing;
                            ...
                            public boolean onFling(
                                    final MotionEvent e2, float velocityX, float velocityY) {
                                if (mEmulator == null) return true;
                                // Do not start scrolling until last fling has been taken care of:
                                if (!mScroller.isFinished()) return true;

                                final boolean mouseTrackingAtStartOfFling =
                                        mEmulator.isMouseTrackingActive();
                                float SCALE = 0.25f;
                            ...
                            public boolean onDown(float x, float y) {
                                ...
                                // errors when long pressing in terminal view text area
                                // ViewDragHelper: Ignoring pointerId=0 because ACTION_DOWN was not
                                // received for this pointer before ACTION_MOVE
                                // Commenting out the call to mGestureDetector.onTouchEvent(event)
                                // in GestureAndScaleRecognizer#onTouchEvent() removes
                            ...
                            public boolean onDoubleTap(MotionEvent event) {
                                // Do not treat is as a single confirmed tap - it may be followed by
                                // zoom.
                                return false;
                            }
    ...
    /**
     * @param client The {@link TerminalViewClient} interface implementation to allow for
     *               communication between {@link TerminalView} and its client.
     */
    ...
    public InputConnection onCreateInputConnection(EditorInfo outAttrs) {
        ...
        if (mClient.isTerminalViewSelected()) {
            if (mClient.shouldEnforceCharBasedInput()) {
                ...
                // W/InputAttributes: Unexpected input class: inputType=0x00080090
                // imeOptions=0x02000000
                ...
                // Using InputType.NULL is the most correct input type and avoids issues with other
                // hacks.
                //
     ...
     * when context menu for the {@link TerminalView} is started by {@link
     * TextSelectionCursorController#ACTION_MORE} is closed.
     */
    public void onContextMenuClosed(Menu menu) {
        // Unset the stored text since it shouldn't be used anymore and should be cleared from
        // memory
    ...
    public void setTextSize(int textSize) {
        mRenderer =
                new TerminalRenderer(
                        textSize, mRenderer == null ? Typeface.MONOSPACE : mRenderer.mTypeface);
        updateSize();
    }
     ...
     * @param relativeToScroll If true the column number will take the scroll position into account.
     *                         E.g. if scrolled 3 lines up and the event position is in the top left, column will be -3
     *                         if relativeToScroll is true and 0 if relativeToScroll is false.
    ...
    public int[] getColumnAndRow(MotionEvent event, boolean relativeToScroll) {
        ...
        int row =
                (int)
                        ((event.getY() - mRenderer.mFontLineSpacingAndAscent)
    ...
    /**
     * Key presses in software keyboards will generally NOT trigger this listener, although some may
     * elect to do so in some situations. Do not rely on this to catch software key presses. Gboard
     * calls this when shouldEnforceCharBasedInput() is disabled (InputType.TYPE_NULL) instead of
     * calling commitText(), with deviceId=-1. However, Hacker's Keyboard, OpenBoard, LG Keyboard
     * call commitText().
     *
     * <p>This function may also be called directly without android calling it, like by
     * `TerminalExtraKeys` which generates a KeyEvent manually which uses {@link
     * KeyCharacterMap#VIRTUAL_KEYBOARD} as the device (deviceId=-1), as does Gboard. That would
     * normally use mappings defined in `/system/usr/keychars/Virtual.kcm`. You can run `dumpsys
     * input` to find the `KeyCharacterMapFile` used by virtual keyboard or hardware keyboard. Note
     * that virtual keyboard device is not the same as software keyboard, like Gboard, etc. Its a
     * fake device used for generating events and for testing.
     *
     * <p>We handle shift key in `commitText()` to convert codepoint to uppercase case there with a
     ...
     * `mClient.readShiftKey()`, based on value in kcm files. This may result in different behaviour
     * depending on keyboard and android kcm files set for the InputDevice for the event passed to
     * this function. This will likely be an issue for non-english languages since `Virtual.kcm` in
     * english only by default or at least in AOSP. For both hardware shift key (via
     * effectiveMetaState) and `mClient.readShiftKey()`, `getUnicodeChar()` is used for shift
     * specific behaviour which usually is to uppercase.
     *
     * <p>For fn key on hardware keyboard, android checks kcm files for hardware keyboards, which is
     * `Generic.kcm` by default, unless a vendor specific one is defined. The event passed will have
     * {@link KeyEvent#META_FUNCTION_ON} set. If the kcm file only defines a single character or
     * unicode code point `\\uxxxx`, then only one event is passed with that value. However, if kcm
     * defines a `fallback` key for fn or others, like `key DPAD_UP { ... fn: fallback PAGE_UP }`,
     * then android will first pass an event with original key `DPAD_UP` and {@link
     * KeyEvent#META_FUNCTION_ON} set. But this function will not consume it and android will pass
     * another event with `PAGE_UP` and {@link KeyEvent#META_FUNCTION_ON} not set, which will be
     * consumed.
     *
     * <p>Now there are some other issues as well, firstly ctrl and alt flags are not passed to
     * `getUnicodeChar()`, so modified key values in kcm are not used. Secondly, if the kcm file for
     * other modifiers like shift or fn define a non-alphabet, like { fn: '\u0015' } to act as
     * DPAD_LEFT, the `getUnicodeChar()` will correctly return `21` as the code point but action
     * will not happen because the `handleKeyCode()` function that transforms DPAD_LEFT to `\033[D`
     * escape sequence for the terminal to perform the left action would not be called since its
     * called before `getUnicodeChar()` and terminal will instead get `21 0x15 Negative
     * Acknowledgement`. The solution to such issues is calling `getUnicodeChar()` before the call
     * to `handleKeyCode()` if user has defined a custom kcm file, like done in POC mentioned in
     * #2237. Note that Hacker's Keyboard calls `commitText()` so don't test fn/shift with it for
     * this function. https://github.com/termux/termux-app/pull/2237
     ...
     *
     * <p>Key Character Map (kcm) and Key Layout (kl) files info:
     * https://source.android.com/devices/input/key-character-map-files
     * https://source.android.com/devices/input/key-layout-files
     * https://source.android.com/devices/input/keyboard-devices AOSP kcm and kl files:
     ...
     *
     * <p>Keyboard layouts advertised by applications, like for hardware keyboards via
     * #ACTION_QUERY_KEYBOARD_LAYOUTS Config is stored in `/data/system/input-manager-state.xml`
     * https://github.com/ris58h/custom-keyboard-layout Loading from apps:
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        ...
        final boolean leftAltDown =
                (metaState & KeyEvent.META_ALT_LEFT_ON) != 0 || mClient.readAltKey();
    ...
    public void inputCodePoint(
            int eventSource,
            int codePoint,
            boolean controlDownFromEvent,
            boolean leftAltDownFromEvent) {
        if (TERMINAL_VIEW_KEY_LOGGING_ENABLED) {
            mClient.logInfo(
                    LOG_TAG,
                    "inputCodePoint(eventSource="
                            + eventSource
                            + ", codePoint="
                            + codePoint
    ...
    public void updateSize() {
        ...
        int newRows =
                Math.max(
                        4,
                        (viewHeight - mRenderer.mFontLineSpacingAndAscent)
                                / mRenderer.mFontLineSpacing);
     ...
     *
     * <p>The {@link #setTerminalCursorBlinkerState(boolean, boolean)} must be called after this for
     * changes to take effect if not disabling.
     *
     * @param blinkRate The value to set.
     * @return Returns {@code true} if setting blinker rate was successfully set, otherwise [@code
     * false}.
     */
    public synchronized boolean setTerminalCursorBlinkerRate(int blinkRate) {
        ...
        if (blinkRate != 0
                && (blinkRate < TERMINAL_CURSOR_BLINK_RATE_MIN
                || blinkRate > TERMINAL_CURSOR_BLINK_RATE_MAX)) {
            mClient.logError(
                    LOG_TAG,
                    "The cursor blink rate must be in between "
                            + TERMINAL_CURSOR_BLINK_RATE_MIN
                            + "-"
                            + TERMINAL_CURSOR_BLINK_RATE_MAX
                            + ": "
                            + blinkRate);
    ...
    /**
     * Sets whether cursor blinker should be started or stopped. Cursor blinker will only be started
     * if {@link #mTerminalCursorBlinkerRate} does not equal 0 and is between {@link
     * #TERMINAL_CURSOR_BLINK_RATE_MIN} and {@link #TERMINAL_CURSOR_BLINK_RATE_MAX}.
     *
     * <p>This should be called when the view holding this activity is resumed or stopped so that
     * cursor blinker does not run when activity is not visible. If you call this on onResume() to
     * start cursor blinking, then ensure that {@link #mEmulator} is set, otherwise wait for the
     * {@link TerminalViewClient#onEmulatorSet()} event after calling {@link
     * #attachSession(TerminalSession)} for the first session added in the activity since blinking
     * will not start if {@link #mEmulator} is not set, like if activity is started again after
     * exiting it with double back press. Do not call this directly after {@link
     * #attachSession(TerminalSession)} since {@link #updateSize()} may return without setting
     * {@link #mEmulator} since width/height may be 0. Its called again in {@link
     * #onSizeChanged(int, int, int, int)}. Calling on onResume() if emulator is already set is
     * necessary, since onEmulatorSet() may not be called after activity is started after device
     * display timeout with double tap and not power button.
     *
     * <p>It should also be called on the {@link
     * com.rv882.terminal.TerminalSessionClient#onTerminalCursorStateChange(boolean)} callback when
     * cursor is enabled or disabled so that blinker is disabled if cursor is not to be shown. It
     * should also be checked if activity is visible if blinker is to be started before calling
     * this.
     *
     * <p>It should also be called after terminal is reset with {@link TerminalSession#reset()} in
     * case cursor blinker was disabled before reset due to call to {@link
     * com.rv882.terminal.TerminalSessionClient#onTerminalCursorStateChange(boolean)}.
     *
     * <p>How cursor blinker starting works is by registering a {@link Runnable} with the looper of
     ...
     * on a queue.
     *
     * <p>Note that when moving cursor in text editors like nano, the cursor state is quickly
     * toggled `-> off -> on`, which would call this very quickly sequentially. So that if cursor is
     * moved 2 or more times quickly, like long hold on arrow keys, it would trigger `-> off -> on
     * -> off -> on -> ...`, and the "on" callback at index 2 is automatically cancelled by next
     * "off" callback at index 3 before getting a chance to be run. For this case we log only if
     * {@link #TERMINAL_VIEW_KEY_LOGGING_ENABLED} is enabled, otherwise would clutter the log. We
     * don't start the blinking with a delay to immediately show cursor in case it was previously
     * not visible.
     *
     ...
     *                                 cursor is even enabled by {@link TerminalEmulator} before starting the cursor blinker.
     */
    public synchronized void setTerminalCursorBlinkerState(
            boolean start, boolean startOnlyIfCursorEnabled) {
        ...
        if (start) {
            ...
            if (mTerminalCursorBlinkerRate < TERMINAL_CURSOR_BLINK_RATE_MIN
                    || mTerminalCursorBlinkerRate > TERMINAL_CURSOR_BLINK_RATE_MAX) return;
                // If cursor blinder is to be started only if cursor is enabled
            else if (startOnlyIfCursorEnabled && !mEmulator.isCursorEnabled()) {
                if (TERMINAL_VIEW_KEY_LOGGING_ENABLED)
                    mClient.logVerbose(
                            LOG_TAG,
                            "Ignoring call to start cursor blinker since cursor is not enabled");
                return;
            }
            ...
            if (TERMINAL_VIEW_KEY_LOGGING_ENABLED)
                mClient.logVerbose(
                        LOG_TAG,
                        "Starting cursor blinker with the blink rate "
                                + mTerminalCursorBlinkerRate);
            ...
            mTerminalCursorBlinkerRunnable =
                    new TerminalCursorBlinkerRunnable(mEmulator, mTerminalCursorBlinkerRate);
    ...
    public String getStoredSelectedText() {
        return mTextSelectionCursorController != null
                ? mTextSelectionCursorController.getStoredSelectedText()
                : null;
    }
    ...
    protected void onDetachedFromWindow() {
        ...
        if (mTextSelectionCursorController != null) {
            ...
            // android.view.WindowLeaked: Activity com.termux.app.TermuxActivity has leaked window
        ...
        public void run() {
            try {
                if (mEmulator != null) {
                    ...
                    // mClient.logVerbose(LOG_TAG, "Toggling cursor blink state to " +
                    // mCursorVisible);
