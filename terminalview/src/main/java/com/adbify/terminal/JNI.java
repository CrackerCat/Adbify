 ...
 */
final class JNI {
    static final String LIB_NAME = "adbify-term";

    static {
        try {
            System.loadLibrary(LIB_NAME);
        } catch (UnsatisfiedLinkError e) {
            ReLinker.loadLibrary(TerminalContextProvider.getSContext(), LIB_NAME);
        }
    }
     ...
     * executing.
     *
     * @return if >= 0, the exit status of the process. If < 0, the signal causing the process to
     * stop negated.
     */
